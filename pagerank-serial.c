...
int main(int argc, char* argv[]){
	...
	// build vectors
	double *y0  = malloc(n*sizeof(double)); // holds initial page probabilities
	double *y  = malloc(n*sizeof(double)); // holds intermediate and ultimate probs.
	uniform(y0, n);
	...
	double *C = malloc(S->coldim*sizeof(double));
	...
	//printf("final (page rank) probabilities, %d iterations in time %g\n", iters, elapsed);
	printvec(y, n); printf("\n");

	//start = MPI_Wtime();
	iters = solve(S, C, d, y0, y, eps, serial_mvpSM);
	//elapsed2 = MPI_Wtime() - start;
	//printf("final (page rank) probabilities, %d iterations, speedup over strawman is %g\n", iters, elapsed/elapsed2);
	printvec(y, n);
